...
function isContained(item, possibilities) {
    ...
    for(var i=0; i < possibilities.length; i++) {
        if(item.indexOf(possibilities[i]) > -1) {
            isContained = true;
            break;
        }
    }
...
function isIn(item, possibilities) {
    if(!possibilities) return false;
    var isContained = false;
    if(!angular.isArray(possibilities)) possibilities = [possibilities];
    for(var i=0; i < possibilities.length; i++) {
        if(item === possibilities[i]) {
            isContained = true;
            break;
        }
    }
...
function generateRangeArray(count) {
    var rangeArray = [];
...
//var paragraph = "L'avantage d'utiliser le lorem ipsum est bien �videmment de pouvoir cr�er des maquettes ou de remplir un site internet de contenus qui pr�sentent un rendu s'approchant un maximum du rendu final. \n Par d�faut lorem ipsum ne contient pas d'accent ni de caract�res sp�ciaux contrairement � la langue fran�aise qui en contient beaucoup. C'est sur ce crit�re que nous proposons une solution avec cet outil qui g�n�rant du faux-texte lorem ipsum mais avec en plus, des caract�res sp�ciaux tel que les accents ou certains symboles utiles pour la langue fran�aise. \n L'utilisation du lorem standard est facile d�utilisation mais lorsque le futur client utilisera votre logiciel il se peut que certains caract�res sp�ciaux ou qu'un accent ne soient pas cod�s correctement. \n Cette page a pour but donc de pouvoir perdre le moins de temps possible et donc de tester directement si tous les encodages de base de donn�e ou des sites sont les bons de plus il permet de r�cuperer un code css avec le texte format� !";
